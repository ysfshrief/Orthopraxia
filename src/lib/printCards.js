...
export async function printCards(items, { logoUrl = '/logo.png' } = {}) {
  ...
  const cards = withQr.map(it => `
    <div class="c">
      ...
      ${it.roleLabel ? `<div class="role">${escapeHtml(it.roleLabel)}</div>` : ''}
    </div>`).join('')
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .c{width:240px;border:2px solid var(--gold,#C99A3A);border-color:#C99A3A;border-radius:16px;padding:16px;text-align:center;page-break-inside:avoid;background:linear-gradient(160deg,#fffaf2,#f5e4c8)}
    ...
    .n{font-weight:900;font-size:18px;margin-top:10px;color:#3A241C}
    .role{color:#fff;display:inline-block;padding:4px 14px;border-radius:99px;margin-top:6px;font-size:12px;font-weight:700;background:#6B2318}
    @media print{ body{gap:10px;padding:8px} .c{border-width:1px} }
